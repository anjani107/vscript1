                ...
                <div class="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                    ...
                    <div class="space-y-2">
                        @foreach ($typeAssets as $asset)
                            <div class="flex items-center justify-between bg-gray-800 rounded-lg px-3 py-2.5 group">
                                <div class="min-w-0">
                                    @if ($type === 'photo')
                                        <a href="{{ Storage::url($asset->path) }}" target="_blank"
                                           class="text-sm text-gray-300 hover:text-indigo-400 truncate block transition" title="{{ $asset->original_name }}">
                                            {{ $asset->original_name }}
                                        </a>
                                    @elseif ($type === 'audio')
                                        <p class="text-xs text-gray-400 truncate mb-1.5" title="{{ $asset->original_name }}">{{ $asset->original_name }}</p>
                                        <audio controls class="w-full h-7" style="min-width:0">
                                            <source src="{{ Storage::url($asset->path) }}">
                                        </audio>
                                    @else
                                        <p class="text-xs text-gray-400 truncate mb-1.5" title="{{ $asset->original_name }}">{{ $asset->original_name }}</p>
                                        <video controls class="w-full rounded" style="max-height:120px">
                                            <source src="{{ Storage::url($asset->path) }}">
                                        </video>
                                    @endif
                                    <p class="text-xs text-gray-600 mt-1">{{ $asset->formatted_size }}</p>
                                </div>
                                <form method="POST" action="{{ route('assets.destroy', $asset) }}"
                                      onsubmit="return confirm('Delete this asset?')" class="ml-2 shrink-0">
                                    @csrf @method('DELETE')
                                    <button class="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-300 transition p-1" title="Delete">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
